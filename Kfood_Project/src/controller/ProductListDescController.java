...
@WebServlet("/ProductListDesc")
public class ProductListDescController extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public ProductListDescController() {
		super();
	}
...
// 		limit 값을 걸어놓은 만큼 범위에 해당하는 글만 가져오는 방법
		...
		List<ProductDTO> productList = ProductListAscService.productDescList(startRow, endRow);

//		페이지 계산을 위한 부분
//		최대로 필요한 페이지 갯수 계산
		int maxPage = (int) ((double) listCount / limit + 0.9);

// 		현재 페이지를 보여줄 시작 페이지 번호
		int startPage = (((int) ((double) page / 10 + 0.9)) - 1) * 10 + 1;

// 		현재 페이지를 보여줄 끝 페이지 번호
